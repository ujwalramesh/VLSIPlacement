...
bool Design::get_bounds_info(Index n, Number* x_l, Number* x_u,Index m, Number* g_l, Number* g_u){
        ...
        double XupperBound = maxx-averageClusterWidth;
        double YupperBound = maxy-averageClusterHeight;

        double ConstraintXupperBound = maxx*2;
        double ConstraintYupperBound = maxy*2;
...
bool Design::get_starting_point(Index n, bool init_x, Number* x,bool init_z, Number* z_L, Number* z_U,
        ...
        //     cout << "populating Inital values for cell: " << (*cellPtr).CellGetName();
                x[idx] = (*cellPtr).CellGetXposDbl()/scaleFactor;
                x[idx+1] = (*cellPtr).CellGetYposDbl()/scaleFactor;
...
//                cout << "Changing Xposition for cell: " << (*cellPtr).CellGetName() << " to " << values[idx] << endl;
                   if ((*cellPtr).CellIsTerminal()) continue;     
                (*cellPtr).CellSetXposDbl(x[idx]*scaleFactor);
                (*cellPtr).CellSetYposDbl(x[idx+1]*scaleFactor);
...
bool Design::eval_grad_f(Index n, const Number* x, bool new_x, Number* grad_f){
        ...
        DESIGN_FOR_ALL_CELLS((*this),cellName,cellPtr){
                ...
                double tempDivX;
                double tempDivY;
                cellXpos = cellXpos /scaleFactor;
                cellYpos = cellYpos/scaleFactor;
                ...
                if (debug) {
                        cout << "temp3 " << temp3 << " and temp4 " << temp4;  
                        cout << " cellMaxy " << cellMaxy << " and pinMaxy " <<pinMaxy;
                        cout << " cellMiny " << cellMiny << " and pinMiny " <<pinMiny << endl;
                        cout << " tempDivX " << tempDivX << " tempDivY " << tempDivY << endl;
                         cout << "temp1 " << temp1 << " and temp2 " << temp2;  
                        cout << " cellMaxx " << cellMaxx << " and pinMaxx " <<pinMaxx;
                        cout << " cellMinx " << cellMinx << " and pinMinx " <<pinMinx << endl;
                }
                        idx=idx+2;
        }DESIGN_END_FOR;
