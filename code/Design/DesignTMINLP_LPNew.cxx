...
bool Design::get_constraints_linearity(Index m, Ipopt::TNLP::LinearityType* const_types){
        for (int idx=0;idx<m;idx++){
                const_types[idx]=Ipopt::TNLP::NON_LINEAR;
        }
...
bool Design::eval_jac_g(Index n, const Number* x, bool new_x,
        ...
        if (values==NULL){
               ...
               for (int i=0;i<size;i=i+2){
                       for (int j=0 ;j <size;j=j+2){
                               if (j <=i) continue;
                                        values[idx2]=(1/sqrt(pow(x[i]-x[j],2) + beta))*(2*(x[i]-x[j]))*x[i];
                                        idx2=idx2+1;
                                        values[idx2]=-(1/sqrt(pow(x[i]-x[j],2) + beta))*(2*(x[i]-x[j]))*x[j];
                                        idx2=idx2+1;
                        }
               }
               for (int i=1;i<size;i=i+2){
